...
지시: 이 프로젝트에서 유저가 할 수 있는 주요 행동 3가지를 식별하고, 각 행동에 대해 코드가 실행되는 순서를 추적해줘.
각 행동당 단계는 최대 4개로 제한해.
각 단계마다:
1. 실행되는 파일명과 함수명
2. 핵심 코드 부분만 짧게 (최대 5줄 이내로 요약)
3. 이 코드가 하는 일을 비개발자도 이해할 수 있게 설명
4. 다음 단계로 넘어가는 이유

중요: codeBlock에는 전체 코드를 넣지 말고, 핵심 로직 5줄 이내로 요약해서 넣어.
응답이 너무 길어지면 JSON이 잘리므로 반드시 간결하게 작성해.
